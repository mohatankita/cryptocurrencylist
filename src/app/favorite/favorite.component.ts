import { Component, OnInit, Input, HostListener, EventEmitter, Output, ViewContainerRef } from '@angular/core';
...
export class FavoriteComponent implements OnInit {
  ...
  public checkvalue(event, idvalue) {
    ...
    if (this.coins.length == 2) {
      ...
      this.toast.info("Please select exactly two cryptocurrency to compare");
    }
  }
      ...
      if (this.priceTempData[x].id == idvalue) {
        for (let a in this.favoriteData) {
          if (this.favoriteData[a].id == idvalue) {
            this.toast.info("Already added to Favorites");
            flag++;
          }
        }
        if (flag == 0) {
  ...
  checkfavoriteID(): any {
    ...
    if (favoriteid) {
      ...
      this.toast.info("NO favorites Selected");
    }
  }

  displaycoindetail(e, coinid): any {
    this.router.navigate(['/detail', coinid]);
  }
