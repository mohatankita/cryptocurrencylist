...
@Component({
  selector: 'compare-currency',
  templateUrl: './comparecurrency.component.html',
  styleUrls: ['./comparecurrency.component.css']
})
export class ComparecurrencyComponent {
  ...
  constructor(private cryptoprice: CryptoserviceService, private route: ActivatedRoute, private router: Router) { }

  getSpecificData(coinValue): any {
    this.loading =  true;
    if (this.flag == 1) {
      this.cryptoprice.getprice(coinValue).subscribe(
        coin1Data => {
